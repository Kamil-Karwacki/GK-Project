...
class SoccerEnv
{
  public:
    SoccerEnv()
    {
        ...
        // Używamy tylko areny 0
        m_arena = &m_scene->m_arenas[0];
    }

    // Gym: reset() → obs shape (38,)
    ...
    // Gym: step(action) → (obs, reward, done, info)
    // action shape: (4,) → [moveX, moveY, jump, kick]
    ...
    // Ile wejść / wyjść — żeby Python wiedział rozmiary przestrzeni
  ...
  private:
    ...
    {
        // Te same 38 inputów co w EnemyController::onUpdate
        // Możesz wyciągnąć logikę zbierania inputów do osobnej funkcji
        // i wywołać ją tu oraz w EnemyController
        ...
        // ... wypełnij tak samo jak w EnemyController
    ...
    void applyAction(py::array_t<float> action)
    {
        ...
        footballer->m_input.x = buf(0); // moveX  (-1..1)
        footballer->m_input.y = buf(1); // moveY  (-1..1)
    ...
    float computeReward()
    {
        // Twój obecny reward shaping — delta dystansu piłki,
        // ustawienie, gol itp.
        // Zwracasz float zamiast akumulować w m_fitnessA
        float reward = 0.0f;
